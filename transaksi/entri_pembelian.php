...
<html lang="en">
	<head>
		...
		<title>Entri Pembelian | TOKO OBAT BERIZIN & KOSMETIK RENDY</title>
	...
	<body>
		<div id="wrapper">
			...
			<?php
				...
				$konek=mysql_query('SELECT * FROM detil_pembelian');
					function autonumber($tabel, $kolom, $lebar=0, $awalan=''){
						$query= mysql_query("SELECT id_faktur FROM detil_pembelian ORDER BY id_faktur DESC LIMIT 1");
				...
				if (isset($_POST['tambah'])) {
					...
					$cekid = mysql_query("SELECT * FROM pembelian WHERE id_barang = '$id_barang'");
					if (mysql_num_rows($cekid) <> 0) {
						echo "<script>alert('Barang Sudah Dikeranjang!');window.location='entri_pembelian.php';</script>";
					}else{
						$tambah = mysql_query("INSERT INTO pembelian VALUES('$id_barang','$nama_barang','$harga_beli','$jumlah_barang','$total')");
					}
				}
				...
				if (isset($_POST['submit'])) {
					...
					$getidbarang		= mysql_fetch_array(mysql_query("SELECT id_barang FROM pembelian"));
					$getnamabarang		= mysql_fetch_array(mysql_query("SELECT nama_barang FROM pembelian"));
					$gethargabeli		= mysql_fetch_array(mysql_query("SELECT harga_beli FROM pembelian"));
					$getjumlahbarang	= mysql_fetch_array(mysql_query("SELECT jumlah_barang FROM pembelian"));
					$gettotal			= mysql_fetch_array(mysql_query("SELECT total FROM pembelian"));
					$id_barang		    = $getidbarang['id_barang'];
					$nama_barang		= $getnamabarang['nama_barang'];
					$harga_beli		    = $gethargabeli['harga_beli'];
					$jumlah_barang		= $getjumlahbarang['jumlah_barang'];
					$total				= $gettotal['total'];
					
					$simpan = mysql_query("SELECT * FROM pembelian");
					while ($a=mysql_fetch_row($simpan)){
						mysql_query("INSERT INTO detil_pembelian VALUES ('$tanggal','$id_faktur','$a[0]','$a[1]','$a[2]','$a[3]','$a[4]')");
						mysql_query("UPDATE barang SET stok=stok+'$a[3]' WHERE id_barang='$a[0]'");
					}
					mysql_query("TRUNCATE TABLE pembelian");
					/*echo "<script>alert('Faktur Siap di Cetak!');window.location='#?id_faktur=$id_faktur';</script>";*/
				}
			?>
						...
						<div class="col-lg-12">
							<div class="panel panel-default">
								<div class="panel-heading">
									<strong>Entri Pembelian</strong>
								</div>
								
								<div class="panel-body">
									<div class="row">
										<div class="col-lg-12">
											<form action="entri_pembelian.php" method="POST" enctype="multipart/form-data" class="form-horizontal">
												<div class="form-group row">
													...
													<label class="col-md-2 col-md-offset-1 form-control-label" for="text-input">ID Faktur</label>
													<div class="col-md-2">
														<input type="text" class="form-control" name="id_faktur" value="<?php echo autonumber("detil_pembelian", "id_faktur", 9, "F") ?>" readonly>
													</div>
												</div>
												<hr>
												<div class="form-group row">
												...
												<div class="form-group row">
													...
													<div class="col-md-2">
														<input type="text" id="harga_beli" name="harga_beli" class="form-control" placeholder="Harga Beli" onkeyup="sum()" readonly>
													</div>
													<div class="col-md-2">
													...
													<div class="col-md-10 table-responsive">
														<table class="table">
															...
															<tr>
																<?php
																	$no = 1;
																	$get = mysql_query("SELECT * FROM pembelian");
																...
																<td><a onclick="{ location.href='faktur_act.php?id=<?php echo $tampil['id_barang']; ?>' }" class="btn btn-danger btn-xs">Hapus</a></td>
															</tr>
																<?php } ?>
														</table>
													</div>
												...
												<div class="form-group row">
													...
													<div class="col-md-2">
														<?php $x=mysql_query("select sum(total) as total_beli from pembelian");
															$xx=mysql_fetch_array($x);
														?>
														<b><input style="font-size: 24px" type="text" id="total_belanja" name="total_belanja" class="form-control" placeholder="0" value="<?php echo $xx['total_beli']; ?>" readonly></b>
													</div>
												</div>
												<div class="form-group row">
													<label class="col-md-2 form-control-label" for="text-input"></label>
													<div class="col-md-5">
														<a href="pembelian.php" type="reset" class="btn btn-default">BATAL</a>
												...
												<table width="100%" class="table table-striped table-bordered table-hover" id="dataTablesBarang">
													...
													<tbody>
														...
														<tr>
															...
															<td id='harga_beli_<?php echo $tampil['id_barang'];?>'><?php echo $tampil['harga_beli']; ?></td>
		  ...
		  function validAngka(a)
		{
			...
			function pilihBarang(id_barang){
				...
				harga_beli = $('#harga_beli_'+id_barang).html();
